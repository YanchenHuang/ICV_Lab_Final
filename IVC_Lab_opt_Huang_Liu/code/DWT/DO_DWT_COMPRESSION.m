...
first_frame_rgb = (imread('C:\Users\yanch\Desktop\final_proj\source\sequence_BasketballDrive_bmp\BasketballDrive_0000.bmp'));
%first_frame_rgb=imread("Lena.tif");
second_frame_rgb = double(imread('C:\Users\yanch\Desktop\final_proj\source\sequence_BasketballDrive_bmp\BasketballDrive_0001.bmp'));
...
for scaleIdx = 1 : numel(scales)
    ...
    for L=1:3
        ...
        for i = 2:19
            fprintf("frame____ %d\n_",i);
            if i<10
                current_frame_name = ['C:\Users\yanch\Desktop\final_proj\source\sequence_BasketballDrive_bmp\BasketballDrive_000',int2str(i),'.bmp'];
            elseif 10<=i<20
                current_frame_name = ['C:\Users\yanch\Desktop\final_proj\source\sequence_BasketballDrive_bmp\BasketballDrive_00',int2str(i),'.bmp'];
            else 
                current_frame_name = ['C:\Users\yanch\Desktop\final_proj\source\sequence_BasketballDrive_bmp\BasketballDrive_0020.bmp'];
            end
            ...
            for Layer=1:3
                ...
                if Layer==1
                    bytestream_R = enc_huffman_new(k-(-15000)+1, BinCode_R_1, Codelengths_R_1);
                    bpp_R(Layer) = (numel(bytestream_R)*8) / (numel(current_frame)/3);
                elseif Layer==2
                    bytestream_R = enc_huffman_new(k-(-15000)+1, BinCode_R_2, Codelengths_R_2);
                    bpp_R(Layer) = (numel(bytestream_R)*8) / (numel(current_frame)/3);
                else
                    bytestream_R = enc_huffman_new(k-(-15000)+1, BinCode_R_1, Codelengths_R_1);
                    bpp_R(Layer) = (numel(bytestream_R)*8) / (numel(current_frame)/3);
        ...
        dwt_bitperpixel_av_video(scaleIdx) = mean(bpp);
        dwt_PSNR_av_video(scaleIdx) = sum(PSNR)/numel(PSNR);
    ...
    %%                         SAVE data
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    save dwt_video dwt_PSNR_av_video dwt_bitperpixel_av_video
