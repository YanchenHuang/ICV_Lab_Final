lena_small_rgb = double(imread('./images/lena_small.tif'));
first_frame_rgb = double(imread('./Sequences/foreman20_40_RGB/foreman0020.bmp'));
second_frame_rgb = double(imread('./Sequences/foreman20_40_RGB/foreman0021.bmp'));
...
[bpp_baseline,psnr_baseline] = baseline(lena_small_rgb,first_frame_rgb,second_frame_rgb,scales);
[bpp_pca,psnr_pca] = opt_pca(lena_small_rgb,first_frame_rgb,second_frame_rgb,scales);
[bpp_deblock,psnr_deblock] = opt_deblock(lena_small_rgb,first_frame_rgb,second_frame_rgb,scales);
[bpp_deblock_pca,psnr_deblock_pca] = opt_pca_deblock(lena_small_rgb,first_frame_rgb,second_frame_rgb,scales);

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%                          折线图
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(1);
plot(bpp_baseline,psnr_baseline,'-*b', ...
     bpp_pca,psnr_pca,'-+r'); %线性，颜色，标记
axis.XLim = [0 4.5];
axis.YLim = [24 42];
axis.XTick = 0:0.1:4.5;
axis.YTick = 24:2:42;
title('Optimization with PCA, Foreman sequences')
legend('baseline','optimization PCA');   %右上角标注
xlabel('bitrate[bit/pixel]');  %x轴坐标描述
ylabel('PSNR[dB]'); %y轴坐标描述

figure(2);
plot(bpp_baseline,psnr_baseline,'-*b', ...
     bpp_deblock,psnr_deblock,'-+r'); %线性，颜色，标记
axis.XLim = [0 4.5];
axis.YLim = [24 42];
axis.XTick = 0:0.1:4.5;
axis.YTick = 24:2:42;
title('Optimization with Deblock, Foreman sequences')
legend('baseline','optimization Deblock');   %右上角标注
xlabel('bitrate[bit/pixel]');  %x轴坐标描述
ylabel('PSNR[dB]'); %y轴坐标描述

figure(3);
plot(bpp_baseline,psnr_baseline,'-*b', ...
     bpp_deblock_pca,psnr_deblock_pca,'-+r'); %线性，颜色，标记
axis.XLim = [0 4.5];
axis.YLim = [24 42];
axis.XTick = 0:0.1:4.5;
axis.YTick = 24:2:42;
title('Optimization with Deblock and PCA, Foreman sequences')
legend('baseline','optimization Deblock + PCA');   %右上角标注
xlabel('bitrate[bit/pixel]');  %x轴坐标描述
ylabel('PSNR[dB]'); %y轴坐标描述
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [bitperpixel_av,PSNR_av] = baseline(lena_small_rgb,first_frame_rgb,second_frame_rgb,scales)
...
for scaleIdx = 1 : numel(scales)
    ...
    for i = 1:20
        current_frame_name = ['./Sequences/foreman20_40_RGB/foreman00',int2str(20+i),'.bmp'];
...
function [bitperpixel_av,PSNR_av] = opt_pca(lena_small_rgb,first_frame_rgb,second_frame_rgb,scales)
...
function [bitperpixel_av,PSNR_av] = opt_deblock(lena_small_rgb,first_frame_rgb,second_frame_rgb,scales)
lena_small = ictRGB2YCbCr(lena_small_rgb);
first_frame = ictRGB2YCbCr(first_frame_rgb);
second_frame = ictRGB2YCbCr(second_frame_rgb);
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [bitperpixel_av,PSNR_av] = opt_pca_deblock(lena_small_rgb,first_frame_rgb,second_frame_rgb,scales)
[t_enc,t_off] = my_pca(first_frame_rgb);
lena_small = my_ictRGB2YCbCr(lena_small_rgb,t_enc,t_off);
first_frame = my_ictRGB2YCbCr(first_frame_rgb,t_enc,t_off);
second_frame = my_ictRGB2YCbCr(second_frame_rgb,t_enc,t_off);
...
counter_zze = 1;
counter_zz =1;
while counter_zz <= length(zz)
    if zz(counter_zz) ~= 0
        zze(counter_zze) = zz(counter_zz);
        counter_zze = counter_zze+1;
        counter_zz = counter_zz+1;
    else
        zze(counter_zze) = 0;
        mask = 0;
        while counter_zz<=length(zz) && zz(counter_zz) == 0 && mask<256
            mask = mask +1;
            counter_zz = counter_zz+1;
        end
        zze(counter_zze+1) = mask-1;
        counter_zze = counter_zze+2;
    end
end
end

function dst = ZeroRunDec(src)
%  Input         : src (zero run encoded sequence 1xM)
%
%  Output        : dst (reconstructed zig-zag scanned sequence 1xN)
counter_src = 1;
dst = [];
counter_dst = 1;
while counter_src <= length(src)
    if src(counter_src) ~= 0
        dst(counter_dst) = src(counter_src);
        counter_dst = counter_dst+1;
        counter_src = counter_src+1;
    else
        for i = 0:src(counter_src+1)
            dst(counter_dst+i) = 0;
        end
        counter_dst = counter_dst+i+1;
        counter_src = counter_src+2;
    end
end
end
